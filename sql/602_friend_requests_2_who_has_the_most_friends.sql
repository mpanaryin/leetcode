...
SELECT combined_id AS id, combined_count AS num FROM (
    SELECT combined_id, combined_count, RANK() OVER(ORDER BY combined_count DESC) AS rank FROM (
        SELECT combined_id, COUNT(*) AS combined_count FROM (
            SELECT requester_id AS combined_id FROM RequestAccepted
            UNION ALL
