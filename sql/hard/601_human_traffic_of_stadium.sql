...
--
--
-- Write a solution to display the records with three or more rows with consecutive id's, and the number
...
WITH
    ...
    GroupedStadium AS (
        SELECT grp FROM StadiumWithGroup
        GROUP BY grp HAVING COUNT(grp) >= 3
    )

SELECT id, visit_date, people
FROM StadiumWithGroup s
JOIN GroupedStadium g ON s.grp = g.grp
