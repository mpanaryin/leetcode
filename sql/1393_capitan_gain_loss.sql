...
--1
SELECT t.stock_name, MAX(t.gain_stock) - MAX(t.loss_stock) AS capital_gain_loss FROM (
    SELECT stock_name,
        SUM(price) FILTER (WHERE operation = 'Buy') AS loss_stock,
        SUM(price) FILTER (WHERE operation = 'Sell') AS gain_stock
    FROM Stocks
