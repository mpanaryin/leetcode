...
-- at time_stamp and that confirmation message was either confirmed ('confirmed') or
-- expired without confirming ('timeout').
--
--
-- The confirmation rate of a user is the number of 'confirmed' messages divided by the total number
-- of requested confirmation messages. The confirmation rate of a user that did not request
...
SELECT DISTINCT s.user_id,
                COALESCE(ROUND(COUNT(c.action) FILTER (WHERE c.action = 'confirmed') * 1.0
