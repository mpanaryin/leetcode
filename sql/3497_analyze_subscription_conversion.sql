...
SELECT user_id,
    ROUND(MAX(CASE WHEN activity_type = 'free_trial' THEN avg END), 2) AS trial_avg_duration,
    ROUND(MAX(CASE WHEN activity_type = 'paid' THEN avg END), 2) AS paid_avg_duration
FROM (
    SELECT user_id, activity_type, AVG(activity_duration) as avg FROM UserActivity
    GROUP BY user_id, activity_type HAVING activity_type IN ('free_trial', 'paid')
) t
GROUP BY user_id HAVING ROUND(MAX(CASE WHEN activity_type = 'paid' THEN avg END), 2) IS NOT NULL