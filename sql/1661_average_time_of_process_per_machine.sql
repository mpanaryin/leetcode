...
-- First solution
WITH MachineProcessGroupWithTotalTime AS (
    SELECT machine_id, ROUND((MAX(timestamp) - MIN(timestamp))::numeric, 3) AS processing_time
    FROM Activity
...
FROM MachineProcessGroupWithTotalTime
GROUP BY machine_id
