...
-- A category pair is considered reportable if at least 3 different customers have purchased products
...
-- Total: 3 customers purchased from both categories
-- Other category pairs like Clothing-Sports (only 2 customers: Users 1 and 4) and Books-Kitchen
-- (only 1 customer: User 3) have fewer than 3 shared customers and are not included in the result.
...
WITH JoinedProductPurchases AS (
    SELECT pp.user_id, pp.product_id, pp.quantity, pi.category FROM ProductPurchases pp
    JOIN ProductInfo pi ON pi.product_id = pp.product_id
)

SELECT
    jpp1.category AS category1,
    jpp2.category AS category2,
    COUNT(DISTINCT jpp1.user_id) AS customer_count
FROM JoinedProductPurchases jpp1
JOIN JoinedProductPurchases jpp2 ON jpp1.user_id = jpp2.user_id AND jpp1.category < jpp2.category
GROUP BY jpp1.category, jpp2.category
HAVING COUNT(DISTINCT jpp1.user_id) >= 3
